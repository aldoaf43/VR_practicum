...
public class mover_tapa_5 : MonoBehaviour
{
    ...
    //diferencia entre cada tapa: 0.0040845
    //diferencia aplicada: 0040845
    //diferencia de tapas real:0.056667
    // Update is called once per frame
    void Update()
    {
        float speed=100f;
        pos_inicial=rb.transform.position;
        Vector3 pos_valvula= new Vector3(seguir_valvula.objeto.x+inicial.x-0.225f,seguir_valvula.objeto.y+dif.y+(0.056667f*2),seguir_valvula.objeto.z-dif.z+0.01f);
        if(mover_tapa==1 && rotate.contar%2==0){
            rb.constraints=RigidbodyConstraints.FreezeRotation;
            rb.constraints=RigidbodyConstraints.FreezePositionX;
            rb.constraints=RigidbodyConstraints.FreezePositionZ;
        }else if (mover_tapa==1 && rotate.contar<2){
